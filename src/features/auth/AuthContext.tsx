...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signInWithGoogle = useCallback(async (): Promise<string | null> => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        // Redirect back to the app root (before the hash)
        redirectTo: window.location.href.split('#')[0],
      },
    });
    if (error) return hebrewAuthError(error);
    return null; // redirect happens — won't reach here normally
  }, []);
...
// ---------------------------------------------------------------------------
// Hook — co-located with provider by convention; disable fast-refresh warning
// ---------------------------------------------------------------------------
